...
public class TicTacToeView extends View {
    ...
    public void processMove() {
        ...
        for (MatOfPoint c : contours) {
            ...
            for (Point p : pList)
                if (!contourPts.contains(p)) contourPts.add(p);
        }
        allContours.fromList(contourPts);

        Point cog = centerOfGravity(allContours);

        int cellNum = -1;
        for (int i = 0; i < cells.length; i++) {
            if (convexPoints.length > 0 && cog.inside(cells[i])) {
                cellNum = i;
                break;
            }
        }
        ...
        for (Point p : contourPts) {
            if (!p.inside(cells[cellNum])) {
                countOutside++;
            }
        }
        double percentOutside = countOutside / contourPts.size();
        if (percentOutside > .3) {
            new Handler(Looper.getMainLooper()).post(new Runnable() {
                @Override
                public void run() {
